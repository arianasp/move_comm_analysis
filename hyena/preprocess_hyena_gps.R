...
#PARAMETERS
max.interp <- 5
max.speed.quantile <- 0.9995 #corresponds to 14.8 m/s
max.move <- 5
max.move.time <- 5*60 
max.dist.quantile <- .9999
min.dist.quantile <- .0001
...
#dir <- '~/Dropbox/hyenas/hyena_data/RData'
...
#Find and remove unrealistic locations (>mean + 10*sd for each ind in x or y)
for(i in 1:n.inds){
  xi <- xs[i,]
  yi <- ys[i,]
